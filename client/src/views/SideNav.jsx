...
function SideNav({handlePassedData, id}) {
    ...
    const handleLogOut = (e) => {
        ...
        handlePassedData({passedData: {}, state: 'userInfo'});
        handlePassedData({passedData: false, state: 'loggedIn'})
    }

    var profile = useRef(null);
    var bookmark = useRef(null);
    var home = useRef(null);

    // pass in endpoint to add style based on window location
    function styleEndPoint(endpoint) {
        return endpoint.current.firstChild.style.fill = '#8691df',
               endpoint.current.lastChild.style.color = '#8691df';
    }

    useEffect(() => {
        var currentUrl = window.location.toString('').split('/');
        var endpoint = currentUrl[3];
        if(endpoint === 'user' && profile !== null) {
            styleEndPoint(profile);
        } else if(endpoint === 'bookmarks') {
            styleEndPoint(bookmark);
        } else if(endpoint === '') {
            styleEndPoint(home);
        }
    }, [])
